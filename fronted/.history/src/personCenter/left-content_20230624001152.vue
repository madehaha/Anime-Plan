          ...
          <div class="content">
            <el-scrollbar max-height="400px">
              <div class="show" v-for="(item,index) in shelvedList">
                <img :src="shelvedList1[index]?.image||'../src/assets/baokemeng.png'" alt="" />
...
service.get('/4/collection').then(res=>{
  ...
  for(let i=0;i<shelvedList.value.length;i++){
    service.get(`/subject/${shelvedList.value[i].id}`).then(res=>{
      
      shelvedList1.value[i].image=res.data.data.Info.image;
      shelvedList1.value[i].name=res.data.data.Info.name;
      console.log(shelvedList1.value[i]);
    })
  }
}).catch(err=>{
  console.log(err);
