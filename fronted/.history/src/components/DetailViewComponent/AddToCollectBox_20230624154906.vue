<template>
  ...
  const postTo = async (x1,x2,x3,x4,x5) => {
    await service
      .patch(`/collection/${localStorage.current_item_id}`,{
        type:x1,
        has_comment:x2,
        comment:x3,
        score:x4,
  ...
  const AddSubmit = () => {
    ...
    var sc=parseInt(value.value)
    sc=sc+sc;
    console.log(search_content[0].value+typeof(search_content[0].value));
    console.log(value.value+typeof(value.value));
    console.log(parseInt(now_charpter[0].value)+typeof(parseInt(now_charpter[0].value)));
     postTo(parseInt(radio.value),true,search_content[0].value,sc,parseInt(now_charpter[0].value));
    }
